...
To encrypt a file then delete securely:

    $ gpg-encrypt foo  &&
      shred -uvz foo

To encrypt a directory then  delete securely:

    $ tar -c foo | 
      gpg-encrypt --output foo.tar.gpg && 
      find foo -type f -exec shred -uvz {} \;
...
Caution: this script deliberately uses the option --pinentry-mode=loopback
which does weaken passphrase handling. This is because the option instead
routes the passphrase through the terminal/agent in a way that can expose
it to shell history, process listings, or scripted --passphrase misuse,
and disables the secure pinentry dialog.

This means the passphrase may be observable, and the script should not be
combined with --passphrase/--passphrase-file on the command line.

We do this because we use this tool with terminals that don't have pinentry,
and also with terminals that have pinentry that doesn't work reliably.

## Caution: Option Overrides

This tool provides its options to gpg, and the user can override them by
providing their own options. This is deliberate to increase user choice.

For example: a user can override the hardened settings by appending
the user's explicit options, such as --cipher-algo idea.

## Caution: shred

The command shred provides data destruction by overwriting the file with
random data before deleting it. It is a good idea to shred files after
encrypting them, to prevent recovery of the original unencrypted data.

However, shred assumes that the file system overwrites data in-place on disk,
which excludes journaled filesystem (ext3/4, XFS, etc) and copy-on-write
filesystems (btrfs, zfs, etc.). This means that shred may not work on a
per-file basis for your filesystem.

Options we use:

- -u ensures that after the shred, the file is unallocated and removed.

- -v enables verbose output for tracking the shred progress.

- -z does a final zeroization to hide that the allocation on disk was shredded.
